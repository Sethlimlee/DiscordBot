    ...
    switch (cmd) {
          ...
          .then(response => {
            if (response.data.data.children[0]) {
              ...
              if (stats.stats) {
                ...
                bot.sendMessage({
                  to: channelID,
                  message: discordMessage + os.EOL + os.EOL + "**Current Sopa Bot Commands: !Cod + Name#platform OR Name#battle.net ID (Calingo93#psn warrenjeffzz#xbl or SopaGrande#1490), !Apex + Origin Username, !Natalie, !Eddie**"
                });
              }
            } else {
              bot.sendMessage({
      ...
      case "cod":
        if (name) {
          ...
          bot.sendMessage({
            to: channelID,
            message: "Please enter a valid Name after !cod (EX: !cod Calingo93#psn warrenjeffzz#xbl or SopaGrande#1490)" +
              os.EOL + os.EOL + "**Current Sopa Bot Commands: !Cod + Name#platform OR Name#battle.net ID (Calingo93#psn warrenjeffzz#xbl or SopaGrande#1490), !Apex + Origin Username, !Natalie, !Eddie**"
          });
          break
        };
